...
extern StringEntry lovrHandPose[];
...
extern StringEntry lovrProjectionType[];
...
void luax_registerloader(lua_State* L, int (*loader)(lua_State* L), int index, int upvalues);
...
bool luax_checkbufferdata(lua_State* L, int index, const struct DataField* format, char* data);
