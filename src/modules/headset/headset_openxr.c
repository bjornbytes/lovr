...
static Texture* openxr_setBackground(uint32_t width, uint32_t height, uint32_t layers) {
  Swapchain* swapchain = &state.swapchains[BACKGROUND];
  ...
  if (!swapchain_init(swapchain, width, height, stereo, depth, cube, immutable)) {
    return NULL;
  }
  ...
  return state.swapchains[BACKGROUND].textures[0];
}
...
static Layer* openxr_newLayer(const LayerInfo* info) {
  ...
  if (!swapchain_init(&layer->swapchain, info->width, info->height, info->stereo, false, false, info->immutable)) {
...
static bool openxr_submit(void) {
  ...
  if (state.frameState.shouldRender) {
    ...
    // Background layer
    if (state.swapchains[BACKGROUND].handle) {
      ...
      swapchain_release(&state.swapchains[BACKGROUND]);
    }
