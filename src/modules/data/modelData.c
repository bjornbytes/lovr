...
void lovrModelDataAllocate(ModelData* model) {
  ...
  totalSize += sizes[3] = ALIGN(model->samplerCount * sizeof(ModelSampler), alignment);
  totalSize += sizes[4] = ALIGN(model->materialCount * sizeof(ModelMaterial), alignment);
  totalSize += sizes[5] = ALIGN(model->attributeCount * sizeof(ModelAttribute), alignment);
  totalSize += sizes[6] = ALIGN(model->primitiveCount * sizeof(ModelPrimitive), alignment);
  ...
  totalSize += sizes[11] = ALIGN(model->childCount * sizeof(uint32_t), alignment);
  ...
  model->samplers = (ModelSampler*) (p + offset), offset += sizes[3];
  model->materials = (ModelMaterial*) (p + offset), offset += sizes[4];
  model->attributes = (ModelAttribute*) (p + offset), offset += sizes[5];
  model->primitives = (ModelPrimitive*) (p + offset), offset += sizes[6];
