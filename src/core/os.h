...
typedef enum {
  ...
  OS_EVENT_FOCUS,
  OS_EVENT_RESIZE,
  OS_EVENT_KEY_PRESSED,
  OS_EVENT_KEY_RELEASED,
  OS_EVENT_TEXT_INPUT,
  OS_EVENT_MOUSE_PRESSED,
  OS_EVENT_MOUSE_RELEASED,
  OS_EVENT_MOUSE_MOVED,
  OS_EVENT_WHEEL_MOVED,
  OS_EVENT_PERMISSION
} os_event_type;
...
typedef union {
  ...
  struct { os_key code; uint32_t scancode; bool repeat; } key;
  struct { uint32_t codepoint; } text;
  struct { int button; } mouse;
  struct { double x, y; } move;
  struct { double dx, dy; } scroll;
  struct { os_permission type; bool granted; } permission;
} os_event_data;
...
typedef void fn_event(os_event_type type, os_event_data* data);
